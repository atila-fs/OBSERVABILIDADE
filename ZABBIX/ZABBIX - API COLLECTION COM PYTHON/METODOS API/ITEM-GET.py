...
api_request = make_request('item.get', api_params, auth_token)

# VERIFICA A RESPOSTA IMPRIME O RESULTADO / TRATA OS POSSIVÉIS ERROS E IMPRIME OS MESMOS
if 'result' in api_request:
    ...
    if debug:
        ...
        for host in result_list:
            print(f"Dado: {host['name']}")
            print(f"Item ID: {host['itemid']}")
            print(f"Chave de Coleta: {host['key_']}")
            print(f"LATEST DATA: {host['lastvalue']}")
            print('----')
else:
