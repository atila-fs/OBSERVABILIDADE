...
# LLD / Retorna todos volumes disponiveis no Pure Storage Flash Array 
def pure_volume_lld():
    try:
        # Pega os argumentos do Zabbix 
        ip = str(sys.argv[2]) #IP do Pure Storage Array
...
# Printa na console os dados do LLD 
        json_data = '{\t"data":['
        temp = 1
        for i in volumeList:
        ...
        print("Erro ao obter informações de volumes : {0}".format(err.reason))    
    except purestorage.PureError as err:
        print("Erro ao obter informações de volumes : {0}".format(err.reason))
    except ValueError:
        print("Erro ao obter informações de volumes devido a um erro com os argumentos (Token,IP)")
    except Exception as e:
        print("Erro desconhecido ao obter informações de volumes : "+str(e))


# Esse module pega as seguintes informações do Pure Storage Array: 
#    * Capacidade
#    * Hostname
#    * Tamanho dos Volumes 
...
# Mostra o output no console
        FNULL = open(os.devnull, 'w')

# Envia os dados para o Zabbix server via Zabbix sender e cria os itens com Zabbix metrics
        metrics = []
        ...
        data.send(metrics)
# Printa (1) sempre que um dado for enviado e consequentemente o respectivo item criado com sucesso
        print(1)

    except Exception as e:

# Printa (0) sempre que um dado for enviado e consequentemente o respectivo item não for criado ou for dado como não suportado   
...
# Esse module tem a intenção de pegar as informações de volume do Pure Storage:
#    * Microssegundo por operação de gravação
#    * Saída por segundo
#    * San Microssegundo por operação de leitura
#    * Ler por segundo
#    * Entrada por segundo
#    * Microssegundo por operação de leitura
#    * San microssegundo por operação de gravação
...
# Esse module tem a intenção de pegar as informações de Array do Pure Storage:
...
# LLD / Descobre todos os hosts disponiveis no Pure Storage Flash Array
...
# Printa na console os dados do LLD 
        ...
        print("Erro ao obter informações de hosts : {0}".format(err.reason))    
    except purestorage.PureError as err:
        print("Erro ao obter informações de hosts : {0}".format(err.reason))
    except ValueError:
        print("Erro ao obter informações de hosts devido a um erro com os argumentos (Token,IP)")
    except Exception as e:
        print("Erro desconhecido ao obter informações de hosts : "+str(e))


# Esse module tem a intenção de pegar as informações de monitoramento de host do Pure Storage : 
#    * Gravações por segundo
#    * Microssegundo por operação de gravação
#    * Saída por segundo
#    * San Microssegundo por operação de leitura
...
# LLD / Descobre todos os discos e NVRAM disponiveis no Pure Storage Flash Array
...
# Printa na console os dados do LLD 
        ...
        print("Erro ao obter informações dos discos : {0}".format(err.reason))    
    except purestorage.PureError as err:
        print("Erro ao obter informações dos discos : {0}".format(err.reason))
    except ValueError:
        print("Erro ao obter informações dos discos devido a um erro com os argumentos (Token,IP)")
    except Exception as e:
        print("Erro desconhecido ao obter informações de discos : "+str(e))
